              ...
              <div className="nav-user">
                {auth?.getUsername()
                  ? <figure onClick={() => { navigate('/home') }}>
                    ...
                    <figcaption><b className={auth?.loggedIn?.fullName.length as number > 5 ? 'user-name' : ''}>{auth?.getUsername()}</b>{auth?.loggedIn?.role === 'Seller' && <p>{auth?.loggedIn?.role}</p>}</figcaption>
                  </figure>
