...
export default function Bag() {
  ...
  const handleComplit = () => {
    if (auth?.loggedIn?._id) {
      setopenOrderModal({ user: true, text: "Sorry,  this is a study project. 😁" })
      return
    }
    ...
    <div className={pathname === "/bag" ? " bag-container mb-30" : "bag-container"}>
              ...
              <tbody>
                {items.map((item: BasketClient) => (
                  <tr key={item.id}>
                    ...
                    <th>
                      <div>${getPrice(item.product.price, item.product.sales)}</div>
                    </th>
                    <th>
      ...
      {pathname === "/bag" && (
        <div className="result-card">
          <div>
            <b>Subtotal:</b> ${getSubtotal().toFixed(2)}
          </div>
          <div>
            <p>Number of products: {items.length}</p>
          </div>
          <div className="M-btn btn-green" onClick={handleComplit}>
            Complete order
          </div>
        </div>
      )}
      ...
      {!!openOrderModal.text && (
        <OrderModal openOrderModal={openOrderModal} setopenOrderModal={setopenOrderModal} />
      )}
    </div>
  )
}
