...
export default function OrderModal({ openOrderModal, setopenOrderModal }: ModalProps) {
    ...
    return (
        <Modal show={!!openOrderModal.text} onHide={() => setopenOrderModal({ user: openOrderModal.user, text: '' })}>
            <Modal.Header closeButton>
                <Modal.Title>{openOrderModal.text}</Modal.Title>
            </Modal.Header>
            <Modal.Footer>
                {openOrderModal.user ?
                    <Button className="primary" onClick={() => setopenOrderModal({ user: openOrderModal.user, text: '' })}>
                        Close
                    </Button>
                    :
